...
def split_data(json_file: str, train_ratio: float = 0.8, val_ratio: float = 0.1, 
               test_ratio: float = 0.1, seed: int = 42) -> Tuple[int, int, int]:
...
def validate_data_quality(json_file: str) -> Dict[str, any]:
    ...
    stats = {
        ...
        'ratio_kor_eng': np.mean(korean_lengths) / np.mean(english_lengths) if english_lengths else 0
    }
...
def main():
    ...
    if args.tsv_file and args.json_file:
        ...
        if args.split:
            split_data(args.json_file, args.train_ratio, args.val_ratio, args.test_ratio)
        ...
        print("Please specify --tsv-file and --json-file for conversion, or --json-file for validation")

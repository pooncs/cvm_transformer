...
class MultimodalCorpusPreparer:
    ...
    def __init__(self, config: MultimodalCorpusConfig = None):
        ...
        self.audio_sources = {
            ...
            'voxlingua107': {
                ...
                'languages': ['ko', 'en', 'ja', 'zh', 'es', 'fr', 'de', 'ru', 'ar', 'hi']
            },
            'librispeech': {
                'en': "http://www.openslr.org/12/"
            }
        }
                    ...
                    if filename.endswith('.zip'):
                        ...
                        for split in ["train", "dev", "test"]:
                            for lang_file in extract_dir.glob(f"*.{split}.*"):
                                downloaded_files[f"opus_{lang_pair}"].append(str(lang_file))
    ...
    def prepare_multilingual_text_corpus(self, 
                                       corpus_files: Dict[str, List[str]]) -> Dict[str, List[str]]:
        """
        ...
        for source_name, file_paths in corpus_files.items():
            for file_path in file_paths:
                try:
                    ...
                    logger.info(f"Processed {len(processed_lines)} lines from {file_path}")
        ...
        for lang in language_corpora:
            ...
            for line in language_corpora[lang]:
                if (len(line.split()) >= self.config.min_sentence_length and
                    len(line.split()) <= self.config.max_sentence_length):
    ...
    def _is_valid_text_line(self, line: str) -> bool:
        ...
        if any(pattern in line for pattern in ['http://', 'https://', '@', 'tel:', 'fax:']):
            return False
    ...
    def prepare_audio_corpus(self, audio_files: Dict[str, List[str]]) -> Dict[str, List[Dict]]:
        """
            ...
            for i in range(0, len(audio_int16) - frame_size, frame_size):
                ...
                if len(frame) == frame_size:
                    ...
                    if is_speech:
                        ...
                        end_idx = min(len(audio), i + frame_size + int(2.0 * sample_rate))  # 2s padding
                        
                        segment_audio = audio[start_idx:end_idx]
                        duration = len(segment_audio) / sample_rate
                        
                        if (self.config.audio_min_duration <= duration <= 
    ...
    def prepare_image_corpus(self, image_files: Dict[str, List[str]]) -> Dict[str, List[Dict]]:
        """
        ...
        for source_name, file_paths in image_files.items():
            for file_path in file_paths:
                try:
                    ...
                    # Resize if needed
                    if image.width < self.config.image_min_width or image.height < self.config.image_min_height:
                        continue
                    ...
                    try:
                        ...
                        logger.info(f"Processed image {file_path}: {len(extracted_text)} chars, lang: {lang}")
                        
                    except ImportError:
                        logger.warning("LanguageDetector not available for image corpus preparation")
                        continue
    ...
    def create_unified_corpus(self, 
                            text_corpora: Dict[str, List[str]],
                            audio_corpora: Dict[str, List[Dict]],
                            image_corpora: Dict[str, List[Dict]]) -> Dict[str, Dict]:
        """
        ...
        all_languages = set(text_corpora.keys()) | set(audio_corpora.keys()) | set(image_corpora.keys())
        ...
        for lang in all_languages:
            unified_corpus[lang] = {
                ...
                'statistics': {
                    ...
                    'total_samples': (len(text_corpora.get(lang, [])) + 
                                    len(audio_corpora.get(lang, [])) + 
                                    len(image_corpora.get(lang, [])))
                }
            }
            
            logger.info(f"Unified corpus for {lang}: {unified_corpus[lang]['statistics']}")
        
        return unified_corpus
    
    def train_large_vocabulary_tokenizer(self, 
                                       unified_corpus: Dict[str, Dict],
                                       vocab_size: int = None) -> str:
        """
        ...
        logger.info(f"Training tokenizer with {len(all_text_lines)} unique lines, vocab_size={vocab_size}")
    ...
    def run_full_preparation(self) -> Dict[str, str]:
        ...
        unified_corpus = self.create_unified_corpus(text_corpora, audio_corpora, image_corpora)
        ...
        results = {
            ...
            'languages': list(unified_corpus.keys()),
            'total_samples': sum(lang_data['statistics']['total_samples'] 

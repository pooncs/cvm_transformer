...
class MultimodalTrainer:
    ...
    def __init__(
        ...
        self.text_optimizer = optim.AdamW(
            text_model.parameters(), lr=1e-4, weight_decay=0.01
        )
        self.image_optimizer = optim.AdamW(
            image_model.parameters(), lr=1e-4, weight_decay=0.01
        )
        self.audio_optimizer = optim.AdamW(
            audio_model.parameters(), lr=1e-4, weight_decay=0.01
        )
    ...
    def train_epoch(
        self, dataloader: DataLoader, model, optimizer, scheduler, modality: str
    ) -> Dict[str, float]:
        ...
        for batch in progress_bar:
            ...
            if modality == "text":
                outputs = self.text_model(
                    src_tokens, tgt_tokens[:, :-1], src_length, tgt_length - 1
                )

            elif modality == "image":
                images = batch["image"].to(self.device)
                outputs = self.image_model(
                    src_tokens, tgt_tokens[:, :-1], src_images=images
                )

            elif modality == "audio":
                audio = batch["audio"].to(self.device)
                outputs = self.audio_model(
                    src_tokens, tgt_tokens[:, :-1], src_audio=audio
                )
                ...
                img_outputs = self.image_model(
                    src_tokens, tgt_tokens[:, :-1], src_images=images
                )
                aud_outputs = self.audio_model(
                    src_tokens, tgt_tokens[:, :-1], src_audio=audio
                )
            ...
            # Calculate loss
            loss = self.criterion(
                outputs.reshape(-1, outputs.size(-1)), tgt_tokens[:, 1:].reshape(-1)
            )
    ...
    def validate(
        self, dataloader: DataLoader, model, modality: str
    ) -> Dict[str, float]:
        """Validate a model."""
        model.eval()
        total_loss = 0
    ...
    def train_multimodal_system(
        ...
        train_dataset = MultimodalDataset(
            train_data, self.tokenizer, include_images=True, include_audio=True
        )
        val_dataset = MultimodalDataset(
            val_data, self.tokenizer, include_images=True, include_audio=True
        )
        ...
        for epoch in range(num_epochs):
            ...
            text_train_metrics = self.train_epoch(
                train_loader,
                self.text_model,
                self.text_optimizer,
                self.text_scheduler,
            ...
            image_train_metrics = self.train_epoch(
                train_loader,
                self.image_model,
            ...
            audio_train_metrics = self.train_epoch(
                train_loader,
                self.audio_model,
            ...
            # Log epoch results
            self.logger.info(
                f"Text - Train Loss: {text_train_metrics['loss']:.4f}, Train Acc: {text_train_metrics['accuracy']:.4f}, "
                f"Val Loss: {text_val_metrics['loss']:.4f}, Val Acc: {text_val_metrics['accuracy']:.4f}"
            )
            self.logger.info(
                f"Image - Train Loss: {image_train_metrics['loss']:.4f}, Train Acc: {image_train_metrics['accuracy']:.4f}, "
                f"Val Loss: {image_val_metrics['loss']:.4f}, Val Acc: {image_val_metrics['accuracy']:.4f}"
            )
            self.logger.info(
                f"Audio - Train Loss: {audio_train_metrics['loss']:.4f}, Train Acc: {audio_train_metrics['accuracy']:.4f}, "
...
def create_training_data() -> Tuple[List[Dict], List[Dict]]:
    ...
    training_pairs = [
        ...
        {
            "korean": "회의는 오후 3시에 시작됩니다",
            "english": "The meeting starts at 3 PM",
        },
        {
            "korean": "프로젝트 일정을 확인해 주세요",
            "english": "Please check the project schedule",
        },
        {
            "korean": "보고서를 제출해야 합니다",
            "english": "I need to submit the report",
        },
    ...
    validation_pairs = [
        ...
        {
            "korean": "할아버지께 안부를 전해 주세요",
            "english": "Please give my regards to grandfather",
        },

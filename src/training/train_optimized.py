...
from src.models.translation_model import SimpleTranslationModel, EnhancedTranslationModel
...
class OptimizedTranslationDataset(Dataset):
    ...
    def __init__(self, src_file: str, tgt_file: str, tokenizer, config: Dict, is_training: bool = True):
    ...
    def _add_noise(self, tokens: List[int]) -> List[int]:
        ...
        for i in range(len(noisy_tokens)):
            if np.random.random() < 0.1:
                noisy_tokens[i] = np.random.randint(4, vocab_size)  # Avoid special tokens
...
class KnowledgeDistillationLoss(nn.Module):
    ...
    def forward(self, student_logits: torch.Tensor, teacher_logits: torch.Tensor, 
        ...
        kd_loss = self.kl_div(student_probs.view(-1, student_probs.size(-1)),
                             teacher_probs.view(-1, teacher_probs.size(-1))) * (T * T)
...
class OptimizedTrainer:
    ...
    def __init__(self, config: Dict):
        ...
        self.scaler = torch.cuda.amp.GradScaler() if (config.get('use_mixed_precision', True) and device_str == 'cuda') else None
    ...
    def create_dataloaders(self, train_src: str, train_tgt: str, val_src: str, val_tgt: str):
    ...
    def train_step(self, batch: Dict) -> float:
        ...
        if self.scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                if self.config.get('use_knowledge_distillation', False) and self.teacher_model is not None:
                    student_logits = self.model(src, tgt, teacher_forcing_ratio=0.5)
                    with torch.no_grad():
                        teacher_logits = self.teacher_model(src, tgt, teacher_forcing_ratio=1.0)
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                for i in range(predictions.size(0)):
                    ...
                    pred_tokens = [t for t in pred_tokens if t > 3 and t < self.tokenizer.vocab_size()]
                    tgt_tokens = [t for t in tgt_tokens if t > 3 and t < self.tokenizer.vocab_size()]
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                if self.global_step % self.config.get('validation_frequency', 500) == 0:
                    ...
                    if val_results['bleu_score'] > self.best_bleu:
                        ...
                        print(f"  No improvement for {self.patience_counter} validations")
                    
                    # Early stopping
                    if self.patience_counter >= self.config.get('early_stopping_patience', 10):
                        print(f"  Early stopping triggered after {self.patience_counter} validations")
                        break
...
def main():
    ...
    if best_bleu >= 0.99:
        ...
        if best_bleu > 0:
            ...
            print("Need significant improvement to reach 99% target - current BLEU is 0.0")

...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if any(t >= vocab_size for t in src_tokens + tgt_tokens):
            ...
            print(f"Max token: {max(src_tokens + tgt_tokens)}, Vocab size: {vocab_size}")
...
class TransformerNMT(nn.Module):
    ...
    def create_padding_mask(self, lengths: torch.Tensor, max_length: int) -> torch.Tensor:
    ...
    def forward(self, src_tokens: torch.Tensor, tgt_tokens: torch.Tensor, 
                src_lengths: torch.Tensor, tgt_lengths: torch.Tensor) -> torch.Tensor:
        """
        Forward pass.
        ...
        if src_tokens.max() >= self.config.vocab_size:
            print(f"ERROR: Source tokens exceed vocab size! Max: {src_tokens.max()}, Vocab: {self.config.vocab_size}")
            src_tokens = torch.clamp(src_tokens, max=self.config.vocab_size-1)
        
        if tgt_tokens.max() >= self.config.vocab_size:
            print(f"ERROR: Target tokens exceed vocab size! Max: {tgt_tokens.max()}, Vocab: {self.config.vocab_size}")
    ...
    def generate(self, src_tokens: torch.Tensor, src_lengths: torch.Tensor,
                max_length: int = 128, temperature: float = 1.0) -> torch.Tensor:
        """
        ...
        generated_tokens = torch.full((batch_size, 1), self.config.sos_token_id, 
...
def main():
    ...
    # Create datasets
    train_dataset = TranslationDataset(train_file, tokenizer_model, config.max_seq_length)

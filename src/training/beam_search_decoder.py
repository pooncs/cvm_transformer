...
class BeamHypothesis:
    ...
    def __init__(self, tokens: List[int], score: float, attention: Optional[torch.Tensor] = None):
        self.tokens = tokens
        self.score = score
...
class BeamSearchDecoder:
    ...
    def __init__(self, model, tokenizer, beam_size: int = 5, max_length: int = 128, 
                 length_penalty: float = 0.6, temperature: float = 1.0):
        self.model = model
    ...
    def decode_single(self, src_tokens: torch.Tensor, src_length: torch.Tensor) -> Tuple[str, List[float], Optional[torch.Tensor]]:
        ...
        with torch.no_grad():
            ...
            # Create source mask
            src_key_padding_mask = self.model.create_padding_mask(src_length, src_tokens.size(1))
            ...
            memory = self.model.transformer.encoder(src_emb, src_key_padding_mask=src_key_padding_mask)
            ...
            for step in range(self.max_length):
                ...
                for beam in beams:
                    ...
                    tgt_tokens = torch.tensor(beam.tokens, dtype=torch.long).unsqueeze(0).to(device)
                    tgt_length = torch.tensor([len(beam.tokens)], dtype=torch.long).to(device)
                    
                    # Create target mask
                    tgt_key_padding_mask = self.model.create_padding_mask(tgt_length, tgt_tokens.size(1))
                    
                    # Add positional encoding
                    tgt_emb = self.model.embedding(tgt_tokens) * np.sqrt(self.model.d_model)
                    tgt_emb = self.model.pos_encoding(tgt_emb.transpose(0, 1)).transpose(0, 1)
                    tgt_emb = tgt_emb.transpose(0, 1)
                    
                    # Create causal mask
                    tgt_length_current = tgt_tokens.size(1)
                    tgt_mask = self.model.transformer.generate_square_subsequent_mask(tgt_length_current).to(device)
                    ...
                    topk_log_probs, topk_indices = torch.topk(log_probs, k=self.beam_size)
                    ...
                    for i in range(self.beam_size):
                        ...
                        new_score = beam.score + log_prob / self.length_penalty_fn(len(new_tokens))
            ...
            if completed_beams:
                ...
                output_tokens = [t for t in best_beam.tokens if t not in [self.bos_token_id, self.eos_token_id]]
    ...
    def decode_batch(self, src_batch: torch.Tensor, src_lengths: torch.Tensor) -> List[Tuple[str, List[float], Optional[torch.Tensor]]]:
...
class TranslationInference:
    ...
    def translate(self, korean_text: str) -> str:
        ...
        # Tokenize input
        korean_tokens = [self.tokenizer.piece_to_id('<s>')] + self.tokenizer.encode(korean_text) + [self.tokenizer.piece_to_id('</s>')]
        korean_tokens = korean_tokens[:128]  # Truncate if too long
        
        # Pad sequence
        korean_length = len(korean_tokens)
        korean_padded = korean_tokens + [self.tokenizer.piece_to_id('<pad>')] * (128 - korean_length)
    ...
    def translate_batch(self, korean_texts: List[str]) -> List[str]:
        ...
        for text in korean_texts:
            tokens = [self.tokenizer.piece_to_id('<s>')] + self.tokenizer.encode(text) + [self.tokenizer.piece_to_id('</s>')]
            tokens = tokens[:128]
            length = len(tokens)

...
def train_epoch_extended(
    student, teacher, loader, optimizer, device, epoch, total_epochs
):
    student.train()
    teacher.eval()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        ce_loss = torch.nn.functional.cross_entropy(
            s_logits_flat, tgt_flat, ignore_index=0
        )
...
def test_translation_quality(student, tokenizer, device):
    ...
    with torch.no_grad():
        for korean, expected in test_sentences:
            ...
            predicted_words = [
                tokenizer.decode([idx]) if idx < len(tokenizer.vocab) else "<UNK>"
...
def main():
    ...
    student = CVMTransformer(
        vocab_size,
        d_model=d_model,
        n_layers=n_layers_student,
        core_capacity=core_capacity_student,
    ).to(device)
    teacher = CVMTransformer(
        vocab_size,
        d_model=d_model,
        n_layers=n_layers_teacher,
        core_capacity=core_capacity_teacher,
    ).to(device)
    ...
    for epoch in range(total_epochs):
        ...
        # Training
        avg_loss, batch_losses = train_epoch_extended(
            student, teacher, loader, optimizer, device, epoch, total_epochs
        )
        ...
        if (epoch + 1) % 10 == 0:
            ...
            metrics["translation_quality"].append(
                {"epoch": epoch + 1, "results": quality_results}
            )
            ...
            for i, result in enumerate(quality_results[:3]):
                print(
                    f"   {i+1}. '{result['input']}' → '{result['predicted']}' (expected: '{result['expected']}')"
                )
    ...
    print(
        f"   Average Epoch Time: {sum(metrics['training_times'])/len(metrics['training_times']):.2f}s"
    )
    print(f"   Final Loss: {metrics['epoch_losses'][-1]:.6f}")
    print(f"   Initial Loss: {metrics['epoch_losses'][0]:.6f}")
    print(
        f"   Loss Reduction: {((metrics['epoch_losses'][0] - metrics['epoch_losses'][-1]) / metrics['epoch_losses'][0] * 100):.1f}%"
    )

    print(f"\n🚀 PERFORMANCE ANALYSIS:")
    print(
        f"   Training Speed: {len(loader) * len(metrics['epoch_losses']) / total_training_time:.1f} iterations/second"
    )
    ...
    if len(metrics["epoch_losses"]) > 1:
        loss_diff = [
            metrics["epoch_losses"][i] - metrics["epoch_losses"][i + 1]

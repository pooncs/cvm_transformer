...
class TransformerNMT(nn.Module):
    ...
    def create_padding_mask(self, lengths, max_length=None):
        ...
        mask = torch.arange(max_length, device=lengths.device).unsqueeze(0).expand(batch_size, max_length)
    ...
    def forward(self, src, tgt, src_key_padding_mask=None, tgt_key_padding_mask=None,
        ...
        tgt_mask = self.transformer.generate_square_subsequent_mask(tgt_length).to(device)
...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Tokenize
        korean_tokens = [self.bos_token_id] + self.tokenizer.encode(korean) + [self.eos_token_id]
        english_tokens = [self.bos_token_id] + self.tokenizer.encode(english) + [self.eos_token_id]
        ...
        # Pad sequences
        korean_padded = korean_tokens + [self.pad_token_id] * (self.max_length - len(korean_tokens))
        english_padded = english_tokens + [self.pad_token_id] * (self.max_length - len(english_tokens))
...
def train_epoch(model, dataloader, optimizer, criterion, scheduler, epoch, device, grad_accumulation_steps=4):
    model.train()
    total_loss = 0
    total_tokens = 0
    total_correct = 0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        tgt_key_padding_mask = model.create_padding_mask(english_lengths, english.size(1))
...
def main():
    ...
    parser.add_argument('--data-file', default='data/processed_large_simple/train_data.json')
    parser.add_argument('--val-file', default='data/processed_large_simple/val_data.json')
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc = train_epoch(model, train_dataloader, optimizer, 
                                          criterion, scheduler, epoch, device)

...
class WordErrorRate:
    ...
    def _levenshtein_distance_words(
...
class METEORScore:
    ...
    def _calculate_fragmentation(
        self, alignments: List[tuple], ref_length: int
    ) -> float:
    ...
    def __call__(self, reference: str, hypothesis: str) -> float:
        ...
        if precision == 0 or recall == 0:
            ...
            f_mean = (precision * recall) / (
                self.alpha * recall + (1 - self.alpha) * precision
            )
...
class TranslationMetrics:
    ...
    def calculate_batch(self, references: List[str], hypotheses: List[str]) -> dict:
        ...
        for metric in [
            ...
            avg_scores[f"avg_{metric}"] = np.mean(
                [scores[metric] for scores in all_scores]
            )
...
def compute_translation_accuracy(
    predictions: List[str], references: List[str], threshold: float = 0.8
) -> float:
    """

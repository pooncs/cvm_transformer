...
def translate_text(model_or_inference, text: str, tokenizer, use_beam_search: bool) -> str:
    ...
    if use_beam_search:
        ...
        with torch.no_grad():
            ...
            for _ in range(128):
                ...
                # Create masks
                src_mask = model.create_padding_mask(length_tensor, input_tensor.size(1))
                tgt_length = torch.tensor(len(output_tokens), dtype=torch.long).unsqueeze(0).to(device)
                tgt_mask = model.create_padding_mask(tgt_length, tgt_tensor.size(1))
                
                # Forward pass
                output = model(input_tensor, tgt_tensor, src_key_padding_mask=src_mask, tgt_key_padding_mask=tgt_mask, memory_key_padding_mask=src_mask)
...
def run_final_validation(model_path: str, tokenizer_model: str, test_data: str, 
                         accuracy_threshold: float = 99.0, max_samples: int = None):
    ...
    # Load model and tokenizer
    model_or_inference, tokenizer, use_beam_search = load_model_and_tokenizer(model_path, tokenizer_model, device)
    ...
    for i, pair in enumerate(test_pairs):
        ...
        hypothesis = translate_text(model_or_inference, korean, tokenizer, use_beam_search)
    ...
    translation_accuracy = (0.6 * avg_char_acc + 0.3 * avg_bleu + 0.1 * exact_match_rate) * 100
...
def main():
    parser = argparse.ArgumentParser(description='Final Korean-English Translation Validation')
    parser.add_argument('--model-path', default='models/extended/best_model.pt')
    parser.add_argument('--tokenizer-model', default='data/processed_large_simple/sentencepiece_large.model')
    parser.add_argument('--test-data', default='data/processed_large_simple/test_data.json')
    ...
    if not os.path.exists(args.model_path):
        ...
        print("python src/training/train_extended_nmt.py --epochs 50 --quick-validation")
        return 1

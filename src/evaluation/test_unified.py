...
def load_model_universal(model_path: str, device: str):
    ...
    try:
        ...
        has_embedding_scale = any(
            "embedding.weight" in key and "sqrt" in str(value)
...
def translate_universal(
    model, tokenizer, korean_text: str, device: str, max_length: int = 30
):
...
def test_model_universal(model_path: str, model_name: str):
    ...
    try:
        ...
        print(
            f"✅ Loaded SentencePiece tokenizer, vocab size: {tokenizer.vocab_size()}"
        )
    ...
    print(
        f"Tests passed (>0.3 score): {sum(1 for r in results if r['score'] > 0.3)}/{len(results)}"
    )
    print(
        f"Tests with good overlap (>0.5): {sum(1 for r in results if r['score'] > 0.5)}/{len(results)}"
    )
    print(
        f"Perfect translations (>0.8): {sum(1 for r in results if r['score'] > 0.8)}/{len(results)}"
    )
        ...
        json.dump(
            {
                ...
                "vocab_utilization": (
                    len(set(all_tokens)) / vocab_size * 100 if all_tokens else 0
                ),
                "results": results,
            },
            f,
...
def main():
    ...
    print("=" * 60)
    print(
        "This script automatically detects model architecture and tests compatibility"
    )
    ...
    if len(results) > 1:
        ...
        for model_name, result in results.items():
            print(
                f"{model_name}: {result['average_score']:.3f} "

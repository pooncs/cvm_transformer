...
def test_device_handling():
    ...
    try:
        ...
        logger.info(f"Tokenizer test successful: input_ids shape {input_ids.shape}, device {input_ids.device}")
...
def test_distillation_dataset():
    ...
    try:
        ...
        logger.info(f"Input IDs shape: {item['input_ids'].shape}, device: {item['input_ids'].device}")
        logger.info(f"Teacher logits shape: {item['teacher_logits'].shape}, device: {item['teacher_logits'].device}")
        ...
        for key, value in item.items():
            if torch.is_tensor(value):
                ...
                if value.device.type != device.type:
                    logger.error(f"Tensor {key} is on {value.device}, expected {device}")
                    return False

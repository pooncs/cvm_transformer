...
class MultimodalTestSuite:
        ...
        try:
            ...
            return {
                ...
                "multimodal": {
                    ...
                    "fusion": {"embed_dim": 1024, "num_heads": 16, "num_layers": 12},
                },
                "testing": {
                    "batch_size": 8,
                    "max_length": 128,
    ...
    def test_text_only_translation(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for sample in self.text_test_samples:
                ...
                translated_tokens = self.generate_translation(
                    self.text_model, korean_tensor
                )
                ...
                logger.info(
                    f"Text: '{sample['korean']}' -> '{translated_text}' (Expected: '{sample['english']}') BLEU: {bleu:.4f}"
                )
        ...
        summary = {
            ...
            "perfect_translation_rate": results["perfect_translations"]
            / len(self.text_test_samples),
    ...
    def test_multimodal_translation(
        self, modality_combinations: List[str]
    ) -> Dict[str, Dict[str, float]]:
        """Test multimodal translation with different modality combinations"""
        logger.info(
            f"Testing multimodal translation with combinations: {modality_combinations}"
        )
    ...
    def test_specific_modality_combination(self, combination: str) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for sample in self.extended_test_samples:
                ...
                if "image" in combination:
                    image_tensor = self.synthetic_image_data[sample["korean"]].to(
                        self.device
                    )
                    image_input = self.image_encoder(image_tensor)

                if "audio" in combination:
                    audio_tensor = self.synthetic_audio_data[sample["korean"]].to(
                        self.device
                    )
                ...
                translated_tokens = self.generate_multimodal_translation(
                    self.multimodal_model, encoded_features
                )
                translated_text = self.detokenize_text(translated_tokens)
                ...
                logger.info(
                    f"{combination}: '{sample['korean']}' -> '{translated_text}' (Expected: '{sample['english']}') BLEU: {bleu:.4f}"
                )
        ...
        summary = {
            ...
            "average_inference_time": np.mean(results["inference_times"]),
            "perfect_translation_rate": results["perfect_translations"]
            / len(self.extended_test_samples),
    ...
    def generate_translation(
        self, model: nn.Module, input_tokens: torch.Tensor
    ) -> List[int]:
    ...
    def generate_multimodal_translation(
        self, model: nn.Module, encoded_features: torch.Tensor
    ) -> List[int]:
        """Generate translation using multimodal features"""
        ...
        with torch.no_grad():
            ...
            for i in range(min(seq_len, 10)):  # Limit output length
                ...
                token_id = (
                    int(torch.mean(encoded_features[0, i, :100]).item() * 1000) % 1000
                )

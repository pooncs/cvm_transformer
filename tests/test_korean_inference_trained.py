...
class SimpleTokenizer:
    ...
    def _build_vocab(self):
        ...
        korean_syllables = (
            "안녕하세요오늘날씨좋네요실시간번역CVM알고리즘한국어영어감사합니다"
        )
        korean_syllables += (
            "어디가세요이것은테스트입니다좋은아침입니다안녕히가세요네알겠습니다"
        )
        korean_syllables += (
            "아니요괜찮습니다도와주세요얼마예요어디있어요지금몇시예요배고파요"
        )
        korean_syllables += (
            "목말라요피곤해요행복해요슬퍼요하나둘셋오늘은월요일입니다내일만나요"
        )
        korean_syllables += (
            "어제갔어요지금시작합니다이프로그램은한국어를영어로번역하는데도움이됩니다"
        )
        korean_syllables += (
            "CVM변환기는실시간번역에매우효과적인알고리즘입니다우리는빠르고정확한번역"
        )
        ...
        english_words += (
            "WhereareyougoingWhatimeisitnowHowmuchisitWhereisitHelpmeImhungryImthirsty"
        )
        english_words += (
            "ImtiredImhappyImsadonetwothreeTodayisMondaySeeyoutomorrowIwentyesterday"
        )
        english_words += (
            "WestartnowThisprogramhelpstranslateKoreantoEnglishCVMtransformerisavery"
        )
...
def test_korean_inference_comprehensive():
    ...
    for category, test_pairs in test_categories.items():
        ...
        for korean_text, expected_english in test_pairs:
            try:
                ...
                print(
                    f"   Time: {inference_time:.2f}ms | Length: {len(korean_text)} → {len(predicted_text)}"
                )
                print()
    ...
    print(
        f"   Reasonable Outputs: {reasonable_outputs}/{total_tests} ({reasonable_outputs/total_tests*100:.1f}%)"
    )
    ...
    detailed_results = {
        ...
        "quality": {
            ...
            "reasonable_output_percentage": (
                reasonable_outputs / total_tests * 100 if total_tests > 0 else 0
            ),
        },
        "results": results,
    }

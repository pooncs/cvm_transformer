...
try:
    ...
    HAS_METRICS = False
    logging.warning("Some metrics libraries not available. Install with: pip install sacrebleu rouge-score bert-score")
...
class TranslationQualityEvaluator:
    ...
    def __init__(self, config: ValidationConfig):
        self.config = config
        self.rouge_scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'], use_stemmer=True) if HAS_METRICS else None
        
    def compute_comprehensive_metrics(self, hypotheses: List[str], references: List[str]) -> Dict[str, float]:
    ...
    def _compute_fallback_metrics(self, hypotheses: List[str], references: List[str]) -> Dict[str, float]:
...
class QualityDegradationDetector:
    ...
    def detect_degradation(self, current_result: ValidationResult, history: List[ValidationResult]) -> Tuple[bool, str]:
        ...
        if current_result.quality_score < self.config.quality_threshold:
            return True, f"Quality score {current_result.quality_score:.3f} below threshold {self.config.quality_threshold}"
    ...
    def should_rollback(self, current_result: ValidationResult, history: List[ValidationResult]) -> bool:
        ...
        if degradation_detected:
            ...
            if history:
                ...
                if best_quality > current_result.quality_score + self.config.degradation_threshold:
                    return True
...
class DistillationValidator:
    ...
    def __init__(self, config: ValidationConfig, teacher_model: Optional[nn.Module] = None):
        self.config = config
    ...
    def validate_distillation(
        ...
        with torch.no_grad():
            for source, target in validation_data:
                # Student translation
                student_output = self._generate_translation(student_model, source, device)
                student_translations.append(student_output)
                
                # Teacher translation (if available)
                if self.teacher_model:
                    teacher_output = self._generate_translation(self.teacher_model, source, device)
                    teacher_translations.append(teacher_output)
                
                references.append(target)
        
        # Compute metrics
        student_metrics = self.evaluator.compute_comprehensive_metrics(student_translations, references)
        
        teacher_metrics = None
        comparison_ratios = None
        
        if self.teacher_model and teacher_translations:
            teacher_metrics = self.evaluator.compute_comprehensive_metrics(teacher_translations, references)
            comparison_ratios = self._compute_comparison_ratios(student_metrics, teacher_metrics)
        
        # Compute overall quality score
        quality_score = self._compute_quality_score(student_metrics, comparison_ratios)
        
        # Error analysis
        error_analysis = self._perform_error_analysis(student_translations, references, student_metrics)
        ...
        degradation_detected, degradation_reason = self.degradation_detector.detect_degradation(
            validation_result, self.validation_history
        )
        validation_result.degradation_detected = degradation_detected
        
        rollback_recommended = self.degradation_detector.should_rollback(validation_result, self.validation_history)
        ...
        # Log results
        self._log_validation_results(validation_result, degradation_reason, rollback_recommended)
    ...
    def _compute_comparison_ratios(self, student_metrics: Dict[str, float], teacher_metrics: Dict[str, float]) -> Dict[str, float]:
    ...
    def _compute_quality_score(self, metrics: Dict[str, float], comparison_ratios: Optional[Dict[str, float]] = None) -> float:
        """Compute overall quality score"""
    ...
    def _perform_error_analysis(self, hypotheses: List[str], references: List[str], metrics: Dict[str, float]) -> Dict:
        """Perform detailed error analysis"""
        analysis = {
            'total_samples': len(hypotheses),
            'average_length_ratio': np.mean([len(h.split()) / len(r.split()) if r.split() else 0 for h, r in zip(hypotheses, references)]),
    ...
    def _log_validation_results(self, result: ValidationResult, degradation_reason: str, rollback_recommended: bool):
        """Log validation results"""
        logger.info(f"📊 Validation Results (Iteration {result.iteration}):")
        logger.info(f"   Quality Score: {result.quality_score:.4f}")
        logger.info(f"   Best Quality: {self.best_quality:.4f} (Iteration {self.best_iteration})")
        ...
        if self.patience_counter > 0:
            logger.info(f"   Patience counter: {self.patience_counter}/{self.config.early_stopping_patience}")
    
    def should_stop_training(self) -> Tuple[bool, str]:
        """Determine if training should stop early"""
        if self.patience_counter >= self.config.early_stopping_patience:
            return True, f"Early stopping: no improvement for {self.patience_counter} validations"

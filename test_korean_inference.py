...
class SimpleTokenizer:
    ...
    def _build_vocab(self):
        ...
        all_chars = set(korean_chars + english_chars + "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ .,?!")
    ...
    def decode(self, ids):
        return "".join([self.reverse_vocab.get(id, "<unk>") for id in ids if id < len(self.reverse_vocab)])
...
def comprehensive_korean_inference_test():
    ...
    for category, test_pairs in test_categories.items():
        ...
        for korean_text, expected_english in test_pairs:
            try:
                ...
                print(f"   Time: {result['inference_time_ms']:.2f}ms | Length: {result['input_length']} → {result['output_length']}")
                print()
    ...
    # Length analysis
    input_lengths = [r['input_length'] for category_results in results.values() for r in category_results]
    output_lengths = [r['output_length'] for category_results in results.values() for r in category_results]
    ...
    if input_lengths and output_lengths:
        ...
        print(f"   Length Change: {((sum(output_lengths) - sum(input_lengths)) / sum(input_lengths) * 100):+.1f}%")
    ...
    print(f"   Very Long Outputs: {very_long_outputs}/{total_tests} ({very_long_outputs/total_tests*100:.1f}%)")
